...
class _ConfirmingOrderBuyScreenState extends State<ConfirmingOrderBuyScreen> {
  ...
  Future<void> _reject(int orderBuyID, String rejectionReason) async {
    try {
      await ApiBuyOderHistory.updateOrderCusRejecting(
          orderBuyID, rejectionReason);
      await ApiBuyOderHistory.updateStatusOrder(orderBuyID, "REJECTING");
      setState(() {});
    } catch (e) {
      print('Lỗi khi từ chối đặt hàng: $e');
    }
  }

  Future<void> _showRejectionReasonDialog(int orderBuyID) async {
    return showDialog<void>(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Chọn lý do từ chối'),
          content: Column(
            children: rejectionReasons.map((reason) {
              return ListTile(
                ...
                onTap: () async {
                  ...
                  await _reject(orderBuyID, reason);
                  print('orderBuyID: ${orderBuyID}');
                  print('reason: ${reason}');
                },
              );
            }).toList(),
